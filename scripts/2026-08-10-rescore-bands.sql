...
-- New bands: great 7.0-9.9, good 5.0-6.9, not_great 3.0-4.9
-- New rule:  the #1 of a Great list with 5 or more entries scores a flat 10.0,
--            which sits ABOVE the Great band on purpose — it is an award, not
--            the ceiling the spread creeps toward. That is also what keeps #1
--            of a two-venue list at 9.2 rather than 9.3.
--
...
select r.user_id, r.venue_id, r.bucket, r.score
 ...
 where not (
   (r.bucket::text = 'great'     and r.score between 7.0 and 9.9)  or
   -- the awarded top score, which is outside the band by design
   (r.bucket::text = 'great'     and r.rank_position = 0 and r.score = 10.0) or
