...
| 14 | Profile buildout (profile + settings hub) | **PHASE 2 SHIPPED 2026-08-10** (merged `05d0c57`) — settings split to `/settings` behind ☰, age moved into Edit profile, Activity/Tagged tabs on both profiles, rate-on-accept. DDL applied and **proved live 8/8**. A friend's friends list followed 2026-08-11 (merged `aa7f691`, proved 3/3), closing the Friends stat that linked nowhere. Owed: on-device look + a two-account tagged test. Phase 1 shipped 2026-07-19 | Edit profile (name/username/photo), saved spots, age pref, privacy + account sections live on main + production. Avatars bucket live; photo upload verified E2E 2026-07-19. Destination = full IG-style profile/settings → §14 + Decision Log |
...
- **Venue photography — where do real photos come from?** 0/56 venues have an `image_url`; every one shows a category placeholder. Google Places photos carry licensing/attribution terms; own-photos vs venue-supplied vs Google is a real decision, not a build task.
  ...
  **RESOLVED 2026-08-07:** working source is **venue-owned photos** — their own Instagram or website. **Google Places photos stay excluded**, per the 2026-08-05 legal finding below; this decision does not reopen that one. Publisher photos (Yelp, Eater, Time Out) are also excluded — those rights-holders actually enforce. Provenance is stored per-photo in a new `venues.image_source` column (a URL, "supplied by venue," or a photographer credit — never rendered in the app) so a takedown request is a single admin edit, not an investigation. The mechanism to act on this decision shipped the same day on `feat/venue-photos` — see the Decision Log entry below — but it shipped with **0 of 56 venues actually photographed**; sourcing the photos themselves is content work, not code, and remains open.
...
- **Night Recap** — morning-after bars-visited + ranking. ~~Blocked: `checkIn()`/`checkOut()` currently DELETE history~~ **UNBLOCKED 2026-08-05** — `scripts/2026-08-05-saves-and-history-ddl.sql` shipped the delete→expire change (`endActiveCheckIns` now sets `expires_at`/`ended_at`, deletes nothing), and the RLS time-bound fix shipped in the same transaction. `check_ins` already carries `vibe` (5 values) and `would_recommend` (yes/maybe/no) with owner-only UPDATE and trigger-set `vibe_at`, so a partial ratings surface exists. Still open: a per-venue **score** distinct from vibe/recommend, and night-session grouping (check-ins are individual rows; "a night" must be derived). Feasibility prep (2026-07-14): `docs/plans/2026-07-14-night-recap-prep.md`.
...
- **List view: better images and venue detail on the rows — UNBLOCKED
  2026-08-07.** The mechanism now exists (`image_url` renders wherever a venue
  image renders, including list rows); filling it in is content work — sourcing
  and uploading real photos — not a build task. See the venue photography entry
  immediately below and the Decision Log.
- **Venue photography** — verified 2026-07-26: **0 of 56 venues have any
  ...
  **Sourcing decision RESOLVED 2026-08-07** (venue-owned photos; see the open
  question above) **and the admin mechanism to act on it SHIPPED the same day**
  on `feat/venue-photos` — add/replace/remove per venue, plus a bulk
  filename-matching panel. Still **0 of 56 photographed** and the admin UI is
  **not yet verified in a browser** — both open, see the Decision Log.
...
- 2026-08-07 — **Curated venue photos BUILT on `feat/venue-photos`; sourcing decision RESOLVED; admin UI browser verification NOT done, NOT merged.** Closes the "where do real photos come from?" open question above and unblocks §19's "list view: better images." Spec `docs/superpowers/specs/2026-08-07-venue-photos-design.md`, plan `docs/superpowers/plans/2026-08-07-venue-photos.md`, DDL `scripts/2026-08-07-venue-photos-ddl.sql` (pasted, applied, verified live — two new `venues` columns, one bucket row `public = true`). **Decision:** venue-owned photos (their own Instagram or website) are the working source; **Google Places photos stay excluded**, per the 2026-08-05 legal finding — this does not reopen that finding; publisher photos (Yelp, Eater, Time Out) are also excluded, since those rights-holders actually enforce takedowns. Provenance lives per-photo in a new `venues.image_source` column (never rendered in the app) so a takedown request is one admin edit, not an investigation. **What shipped:** `venues.image_url` + `venues.image_source` columns; a public `venue-photos` Storage bucket with admin-only INSERT/UPDATE/DELETE (`src/lib/venuePhotos.ts`, DDL header explains why this bucket is public where `night-photos` is private — a venue photo has no audience to leak past, so the control is write access, not read access); add/replace/remove a photo in the admin venue editor (`VenueEditSheet.tsx`); a thumbnail column and a "no photo" filter in the admin venues table (`AdminVenues.tsx`); a bulk panel (`BulkPhotoPanel.tsx`) that matches selected files to venues by filename and shows a preview table — exact match, duplicate name, no match — that the admin can review before a single confirm click writes anything. Re-encoding reuses the same `reencodeImage()` helper as avatars and night photos, capped at 1200px. **The mechanism ships with 0 of 56 venues photographed.** The app is correct at 0, at 12, or at 56 — every venue without a real photo falls back cleanly to its category placeholder (`src/lib/venueImages.ts`); photographing venues is content work from here, not a build task. **Automated checks are clean** (319 tests, typecheck, production build, schema guard 30/0 drift) **but the admin UI itself has not been verified in a browser** — browser automation is blocked in the environment this was built in and the admin surface requires an admin Google session, so nobody has actually clicked "Add photo," watched a preview render, replaced a photo and confirmed the old file doesn't linger cached, or run the bulk panel against real files. **That verification is the outstanding gate before merge**, not a nice-to-have — this repo has previously shipped components that rendered nothing while every automated check passed. **Upgrade path noted for later:** asking a venue directly for a press shot often works, and swaps in through the same `image_url`/`image_source` admin field with no migration and no code change.

- 2026-08-08 — **Tap-to-expand photos BUILT on `feat/photo-expand`; browser verification NOT done, NOT merged.** Builds directly on the venue-photos work above — with real photos now sourceable, a photo you can't see full-size was the obvious next gap. Spec `docs/superpowers/specs/2026-08-08-photo-expand-design.md`, plan `docs/superpowers/plans/2026-08-08-photo-expand.md`. **What shipped:** tap-to-expand on **all five venue photo surfaces** — the list card thumbnail, saved spots, the venue detail sheet hero, the admin edit-sheet preview, and the admin venues table thumbnail — plus **night-post photos**, migrated onto the same component rather than left on their own path. The lightbox is now **one shared component**, `src/components/PhotoLightbox.tsx`, extracted from what had been inline in `PostCard.tsx`; every call site (`VenuePreview.tsx`, `BarCard.tsx`, `SavedSpotsList.tsx`, `VenueEditSheet.tsx`, `AdminVenues.tsx`, `PostCard.tsx`) renders it the same way. **Only a venue with a real photo is expandable** — a category placeholder is not tappable, via the `hasRealPhoto()` predicate from the venue-photos work — which both avoids popping open a full-screen grey rectangle and keeps the photo from stealing taps on rows that are already tappable elsewhere (the list card, saved-spots row, admin table row all navigate on tap outside the thumbnail). **With 0 of 56 venues photographed as of this writing, the change is invisible in production until photos are added** — same standing condition as the venue-photos entry above. **Lesson that cost a review round:** the plan asserted that clicks inside a Radix portal cannot bubble back to the component that rendered the lightbox, and built the close-on-backdrop-tap behavior on that assumption. **That's false** — React's synthetic events bubble along the React tree the component was rendered from, not the DOM tree the portal mounts into — so tapping the lightbox backdrop was also triggering the venue-card's own click handler underneath and navigating away. Fixed with a `stopPropagation()` inside `PhotoLightbox` itself, so every call site inherits the fix rather than needing its own guard. **Automated checks are clean** (337 tests, typecheck, production build, schema guard 31/0 drift) **but the feature has not been verified in a browser at all** — browser automation is blocked in this environment, and nobody has tapped a real photo on any of the five surfaces, confirmed a placeholder stays inert, checked the saved-spots row still splits correctly into "expand photo" vs. "open venue," or confirmed Escape/backdrop/close-button all dismiss. **That verification is the outstanding gate before merge**, not a nice-to-have, per the same standing rule the venue-photos entry above states.
...
## §34 — Logging a night: one sheet, Beli-shaped (APPROVED 2026-08-12)
